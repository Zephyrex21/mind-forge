...
export default function BottomBar() {
  const { vc } = useTheme();
  const { currentStep, currentStepIndex, steps, canGoNext, goNext, goBack, stepLabel } = useGenerator();

  // Don't show on the generate step (it has its own navigation)
  if (currentStep === 'generate') return null;

  return (
    <div className={`sticky bottom-0 z-20 px-6 py-3 flex items-center justify-between border-t ${vc.surface} backdrop-blur-md`}>
      <button
        onClick={goBack}
